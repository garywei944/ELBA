...
def main(argc, argv):
    ...
    combblas_build_cmd = ["make", "-j", "64", "-C", str(combblas_build_path.absolute())]
    sys.stdout.write(" ".join(combblas_build_cmd) + "\n")
    proc = sp.Popen(combblas_build_cmd)
    proc.wait()

    combblas_install_cmd = ["make", "install", "-j", "64", "-C", str(combblas_build_path.absolute())]
    ...
    elba_build_cmd = ["make", "-j", "64", "-C", str(build_path.absolute())]
